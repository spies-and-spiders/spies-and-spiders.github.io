...
function addMonsterFeatures (mfData) {
	...
	for (let i = 0; i < msbcr.cr.length; i++) {
		...
		$("#msbcr").append(`<tr><td>${curCr._cr}</td><td>${Parser.crToXp(curCr._cr)}</td><td>${curCr.pb}</td><td>${curCr.hpMin}-${curCr.hpMax}</td><td>${curCr.attackBonus}</td><td>${curCr.dprMin}-${curCr.dprMax}</td><td>${curCr.saveDc}</td></tr>`);
	}

	$("#crcalc input").change(calculateCr);
	$("#resistances").change(calculateCr);
	...
	$("#msbcr tr").not(":has(th)").click(async function () {
		...
		const [minHp, maxHp] = $(this).children("td:eq(3)").html().split("-").map(it => parseInt(it));
		...
		$wrpMonFeatures.append(`
			<label class="row crc__mon_feature ui-tip__parent">
				<div class="ve-col-1 crc__mon_feature_wrp_cb">
					<input type="checkbox" id="mf-${Parser.stringToSlug(f.name)}" title="${f.name}" data-hp="${f.hp || ""}" data-dpr="${f.dpr || ""}" data-attackbonus="${f.attackBonus || ""}" class="crc__mon_feature_cb">${numBox}
				</div>
	...
	function parseUrl () {
		if (window.location.hash) {
			...
			$("#dpr").val(curData[1]);
			$("#attackbonus").val(curData[2]);
			if (curData[3] === "true") $("#saveinstead").attr("checked", true);
			changeSize($("#size").val(curData[4]));
			$("#hd").val(curData[5]);
			$("#con").val(curData[6]);
			$("#hp").val(calculateHp());
			if (curData[7] === "true") $("#vulnerabilities").attr("checked", true);
			$("#resistances").val(curData[8]);
...
function calculateCr () {
	...
	const hashParts = [
		...
		$("#hd").val(), // 5
		$("#con").val(), // 6
		$("#vulnerabilities").prop("checked"), // 7
		$("#resistances").val(), // 8
